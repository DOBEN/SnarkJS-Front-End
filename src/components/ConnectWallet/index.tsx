    ...
    return (
        <>
            <div className="flex justify-center">
                <button className='btn btn-blue' onClick={!isConnected ? openConnectModal : () => disconnect()}>
                ...
                <>
                    {chain?.id !== Number(CHAIN_ID) && (
                        <div role="alert" className="error-box">
                            Error: Your wallet is not connected to the correct network. Please switch to the Sepolia testnet.
                        </div>
                    )}
                    <div className="result-box">
                        <h3 className="">Your connected account:</h3>
                        {address !== undefined && (
                            <div>{address}</div>
                        )}
                    </div>
            ...
            <div className="">
                <div id="wallet-connect-modal"
                    className="bg-gray hidden z-1"
                >
                    <div className="text-center border py-4 px-8 rounded">
                        <div className="flex justify-center items-center">
                            ...
                            <br />
                            <br />
                            <button type="button" className="ml-auto border py-2 px-2 rounded hover:bg-lightblue" onClick={() => {
                                ...
                                <li key={connector.id}>
                                    ...
                                    <button key={connector.id} className={`${!connector.ready ? 'opacity-50' : ''}`} onClick={() => connect({ connector })} disabled={!connector.ready}>
                                        <div className={`flex items-center btn btn-blue`}>
                                            ...
                                            <span className="flex-1 ml-3 text-black">{connector.name}{!connector.ready && "(unsupported)"}{isLoading && connector.id === pendingConnector?.id && '(connecting)'}</span>
                                        </div>
                                    </button>
                                </li>
                            ))}
