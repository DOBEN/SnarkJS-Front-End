...
export default function ConnectWallet() {
    ...
    useEffect(() => {
        if (connector !== undefined && chainId !== Number(CHAIN_ID)) {
            if (connector.switchChain != undefined) {
                connector.switchChain(Number(CHAIN_ID)).then(async () => {
    ...
    return (
        <>
            ...
            <div className="flex justify-center">
                {!isConnected && (
                    <button className='text-xl font-semibold text-black py-4 px-6 lg:px-12 navbutton' onClick={openConnectModal}>Connect Wallet</button>
                )}
                {isConnected && (
                    <button className='text-xl font-semibold text-black py-4 px-6 lg:px-12 navbutton' onClick={() => disconnect()}>Disconnect Wallet</button>
                )}
            </div>
            {isConnected && (
                <div className={width < 670 ? 'pt-40 -mt-20' : 'mx-auto max-w-7xl pt-40 px-6 -mt-20'}>
                    {chain?.id != Number(CHAIN_ID) && (
                        <div role="alert">
                            <div className="bg-red text-white justify-center text-center red font-bold rounded-t px-4 py-2">
                                Error
                            </div>
                            <div className="border border-t-0 text-red border-red  text-center justify-center rounded-b bg-red-100 px-4 py-3 text-red-700">
                                <div>Your wallet is not connected to the correct network. Please switch to the Sepolia testnet.</div>
                            </div>
                            <br />
                        </div>
                    )}
                    <div className="table-b bg-navyblue p-8 overflow-x-auto">
                        <h3 className="text-3xl lg:text-5xl  text-offwhite mb-6 text-center sm:text-start flex justify-center">Your connected account</h3>

                        {address !== undefined && (
                            <div className='flex justify-center text-center text-xl font-semibold text-black py-4 px-6 lg:px-12' >{width < 670 ? '0x' + address[2] + address[3] + address[4] + ' ... ' + address[38] + address[39] + address[40] + address[41] : address}</div>
                        )}
                    </div>
                </div>
            )}
            ...
            {/* Modal */}
            <div className={width < 670 ? "mx-auto max-w-7xl -my-10 px-2 relative" : "mx-auto max-w-7xl -my-0 px-2 relative"}>
                <div id="wallet-connect-modal"
                    className="absolute left-1/2 transform -translate-x-1/2  top-1/2 transform -translate-y-1/2 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white hidden z-10"
                >
                    <div className="mt-3 text-center">
                        <button type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="crypto-modal" onClick={() => {
                            let modal = document.getElementById("wallet-connect-modal");
                            if (modal) {
                                modal.style.display = "none"
                            }
                            ...
                            <ul className="my-4 space-y-3">
                                {connectors.map((connector) => (
                                    <div key={connector.id}>
                                        <button className="text-center text-3xl lg:text-5xl font-bold text-offwhite mb-3"
                                            disabled={!connector.ready}
                                            key={connector.id}
                                            onClick={() => connect({ connector })}
                                        >
                                            {connector.name == 'MetaMask' && (
                                                <div className="flex items-center p-3 text-base font-bold rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 ">
                                                    <div>
                                                    ...
                                                    <span className="flex ml-3 whitespace-nowrap text-black">MetaMask
